        ...
        <div class="card-body">
            <form method="POST" action="{{ route('consultorios.update', $reserva) }}" class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <select name="consultorio_numero" class="form-select" required>@for($i=1;$i<=14;$i++)<option value="{{ $i }}" @selected((int) old('consultorio_numero', $reserva->consultorio_numero)===$i)>Cubículo {{ $i }}</option>@endfor</select>
                </div>
                ...
                <div class="col-md-4"><label class="form-label">Estratega</label><select name="estratega_id" class="form-select"><option value="">--</option>@foreach($usuarios as $u)<option value="{{ $u->id }}" @selected((int) old('estratega_id', $reserva->estratega_id)===$u->id)>{{ $u->name }}</option>@endforeach</select></div>
